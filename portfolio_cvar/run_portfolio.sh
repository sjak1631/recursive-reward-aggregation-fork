...
usage() {
    echo "Usage: $0 [--seed SEED] [--adapt_state BOOL] [--adapt_reward BOOL] [--bins N] [--cvar_use_sums BOOL] [--train_objective OBJ]"
    ...
    echo "  --cvar_use_sums    Use sums in CVaR histogram (default: False)"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --cvar_use_sums)  CVAR_USE_SUMS="$2"; shift 2 ;;
...
for SEED in "${SEEDS[@]}"; do
    if [ "$TRAIN_OBJECTIVE" = "cvar" ]; then
        ...
        if [ "$CVAR_USE_SUMS" = "False" ]; then
            SUMS_SUFFIX="_without_sums"
        else
            SUMS_SUFFIX="_sums"
        fi
    else
    ...
    SESSION_NAME="PPO_Portfolio_cvar_ours_multi_env_seed${SEED}_${TRAIN_OBJECTIVE}${BINS_SUFFIX}${SUMS_SUFFIX}"
    LOG_DIR="${SCRIPT_DIR}/workspace/seed${SEED}_${TRAIN_OBJECTIVE}${BINS_SUFFIX}${SUMS_SUFFIX}"
    ...
    if [ $? != 0 ]; then
        ...
        tmux send-keys -t $SESSION_NAME "python ${RUNNER} --seed_start $SEED --adapt_state $ADAPT_STATE --adapt_reward $ADAPT_REWARD --result_dir ${LOG_DIR} --cvar_num_bins $CVAR_NUM_BINS --cvar_use_sums $CVAR_USE_SUMS --train_objective $TRAIN_OBJECTIVE | tee ${LOG_DIR}/ppo_state${ADAPT_STATE}_reward${ADAPT_REWARD}_seed${SEED}_${TRAIN_OBJECTIVE}.log" C-m
